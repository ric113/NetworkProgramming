...
#define COLUMN_NUM 5        /* IP , Port , PatchFileName , SOCKS serverIP , SOCKS serverPort */
...
void SendSOCKSRequest(int,int);

void SendSOCKSRequest(int fd,int hostIndex)
{
    unsigned char package[8];
    
    package[0] = 4;
    package[1] = 1;
    package[2] = hostInfoTable[hostIndex].port / 256 ;
    package[3] = hostInfoTable[hostIndex].port % 256 ;
    
    struct hostent *temp_he;
    if((temp_he = gethostbyname(hostInfoTable[hostIndex].host.c_str())) == NULL){
        cout << "Error: get host by name ! " << endl;
        return;
    }
    hostInfoTable[hostIndex].ip = inet_ntoa(*((struct in_addr *)temp_he -> h_addr));
    
    vector<string> splitedIp = Split(hostInfoTable[hostIndex].ip, '.');
    vector<string>::iterator it = splitedIp.begin();
    int index = 4 ;
    
    while(it != splitedIp.end())
    {
        package[index] = atoi((*it).c_str());
        index ++;
        it++;
    }
    
    write(fd, package, 8);
    read(fd,package,8);    /* 直接read走SOCKS Reply 封包 */
...
void ParseQueryString(string querystring)
{
    ...
    while(itIndex < MAX_SERVER_NUM*COLUMN_NUM)
    {
        for(int i = 0 ; i < COLUMN_NUM ; i++)
        {
            ...
            {
                hostInfoTable[index].socks_host = secondSplit[1];
            }
            else if(i==4)
            {
                hostInfoTable[index].socks_port = atoi(secondSplit[1].c_str());
 ...
 *
 *  Set HostInfo : serv_addr/ *he/SOCKS_ip/sockfd
 */
void SocketCreateAndConnect()
{
    for(int i = 0 ; i < MAX_SERVER_NUM ; i ++)
    {
        if(hostInfoTable[i].isExist)
        {
            if((hostInfoTable[i].he = gethostbyname(hostInfoTable[i].socks_host.c_str())) == NULL){
                cout << "Error: get host by name ! " << endl;
                exit(1);
            }
            ...
            hostInfoTable[i].serv_addr.sin_port = htons(hostInfoTable[i].socks_port); /* 欲使用的Server Port */
            
            /* 將addr轉成字串形式IP  */
            hostInfoTable[i].socks_ip = inet_ntoa(hostInfoTable[i].serv_addr.sin_addr);
            ...
            SendSOCKSRequest(hostInfoTable[i].sockFd,i);
        }
    }
}
...
void ListenSocket()
{
    ...
    bool hasRecvSOCKSReply[5];
    
    for(int i = 0 ; i < 5 ; i ++)
        hasRecvSOCKSReply[i] = false;
            ...
            if(hostInfoTable[i].isExist)
            {
                ...
                if(FD_ISSET(hostInfoTable[i].sockFd,&rstemp))
                {
                    ...
                    if(n>0)
                    {
                        ...
                        if(buffer[i][0] == '%' && buffer[i][1] == ' ')  /* 若是Prompt , Host等待我們輸入    Input , 可先關閉該Hostfd read探針 , 並開啟write探針 */
                        {
                    ...
                    /*
                     ...
                     {
                     // Reply : accept or not ?
                     unsigned char replyBuffer[10001];
                     int n = read(hostInfoTable[i].sockFd,replyBuffer,sizeof(replyBuffer));
                     
                     if(n>0)
                     {
                     if(replyBuffer[1] == 91)
                     {
                     // connect reject by SOCKS server .
                     FD_CLR(hostInfoTable[i].sockFd,&rfds);
                     FD_CLR(hostInfoTable[i].sockFd,&wfds);
                     close(hostInfoTable[i].sockFd);
                     hostCount --;
                     ...
                     hasRecvSOCKSReply[i] = true;
                     }
                     else
                     perror("Read SOCKS reply error!");
